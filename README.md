...
`install.packages(c("dplyr", "haven", "forcats", "ggplot2", "shiny", "posterior", "readr", "here", "stringr", "tibble"))`
...
`analysis/shinyapp.R`. Subnational fitting is included in the app as
well, and in `analysis/bayescoverageapp_subnational.qmd`.

Routine data are not on github, store those in your local
`private/routinedata/` folder. For testing, a routine toy data set is
included in the `data_raw` folder, you need to add `iso`/`admin1` (for
country/subnational estimation) and `indicator_name` to that data set to
use it, see quarto and shiny app for example use.
